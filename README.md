
# Soporte y Mantenimiento de Equipos de Cómputo 2026

## Descripción del Proyecto

Documentación técnica del primer proyecto de **Prácticas Iniciales "F-" (1S2026)** correspondiente a la carrera de **Ingeniería en Ciencias y Sistemas de la Universidad de San Carlos de Guatemala**. Este repositorio detalla el proceso integral de diagnóstico, intervención y optimización de hardware y software, siguiendo estándares de seguridad y buenas prácticas industriales.

---

## Recursos del Proyecto

### Evidencia Audiovisual

Se ha generado un video demostrativo que documenta el procedimiento técnico completo, desde la recepción del equipo hasta las pruebas de rendimiento finales.

* **Enlace al video:** [Ver en YouTube](https://youtu.be/eU--6F3phsE)

### Documentación Técnica

Descripción
 Manual de Mantenimiento	Guía detallada con la metodología paso a paso aplicada al equipo.
 Trifoliar Informativo	Material técnico resumido para consulta rápida de usuario.
 Video Demostrativo	Registro visual de las fases de limpieza y diagnóstico.|

---

## Metodología Aplicada

El proyecto se dividió en tres fases críticas para asegurar la integridad de los componentes:

### 1. Diagnóstico de Sistemas

* Inspección física del hardware y periféricos.
* Análisis de errores críticos en el sistema operativo y tiempos de carga.
* Detección de posibles cuellos de botella térmicos.

### 2. Intervención Técnica (Mantenimiento)

* **Limpieza Interna:** Remoción de partículas sólidas en ventiladores y disipadores mediante aire comprimido.
* **Gestión de Conexiones:** Revisión de cables de datos y alimentación para prevenir fallos por falso contacto.
* **Mantenimiento Preventivo:** Optimización de componentes para evitar degradación futura.

### 3. Validación y Control de Calidad

* Ejecución de pruebas de estrés térmico.
* Verificación de integridad de datos y arranque del sistema.
* Certificación de operatividad del equipo.

---

## Equipo de Trabajo

* Ever Alexander Hernández Lemus
* Marcos Samuel Puluc Coc
* Pablo Daniel Maldonado Gámez
* Carlos Emmanuel Pérez Simón
* Anthony Lopez
* Carlos David Recinos Chip

---

## Objetivos y Conclusiones

El propósito fundamental fue la implementación de técnicas correctivas y preventivas que garanticen la extensión de la vida útil del hardware. Como resultado, se consolidaron competencias en soporte técnico y documentación profesional, cumpliendo satisfactoriamente con los requerimientos académicos del ciclo.

---

**Licencia:** Proyecto con fines exclusivamente académicos para el curso de Prácticas Iniciales "F-" 1S2026.

---
